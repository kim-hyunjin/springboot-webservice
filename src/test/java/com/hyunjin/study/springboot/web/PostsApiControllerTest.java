...
import com.hyunjin.study.springboot.domain.posts.PostsRepository;
...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT) // @WebMvcTest는 JPA기능이 작동하지 않기 때문에 SpringBootTest사용
public class PostsApiControllerTest {
    ...
    @Test
    @WithMockUser(roles = "USER") //인증된 모의 사용자를 만들어 사용. MockMvc에서만 작동.
    public void postsSave() throws Exception {
        ...
        //when
        mvc.perform(post(url) // 생성된 MockMvc를 통해 API테스트 진행. Body본문 영역은 문자열로 표현하기 위해 ObjectMapper를 통해 JSON으로 변환
