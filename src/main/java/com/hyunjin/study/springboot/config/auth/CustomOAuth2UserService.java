...
import com.hyunjin.study.springboot.config.auth.dto.OAuthAttributes;
import com.hyunjin.study.springboot.config.auth.dto.SessionUser;
import com.hyunjin.study.springboot.domain.user.User;
import com.hyunjin.study.springboot.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String userNameAttributeName = userRequest.getClientRegistration().getProviderDetails().getUserInfoEndpoint().getUserNameAttributeName(); // OAuth2 로그인을 진행할때 키가 되는 필드값(Primary Key)

        OAuthAttributes attributes = OAuthAttributes.of(registrationId, userNameAttributeName, oAuth2User.getAttributes()); // OAuthAttributes - OAuth2UserService를 통해 가져온 OAuth2User의 attribute를 담을 클래스
